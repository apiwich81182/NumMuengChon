...
export default function FinancialCalendar({ initialData }: FinancialCalendarProps) {
  ...
  const handleMonthChange = (direction: "prev" | "next") => {
    ...
    startTransition(async () => {
      try {
        const res = await fetch(`/api/admin/calendar?year=${nextYear}&month=${nextMonth}`);
        if (res.ok) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-4 sm:p-6 lg:p-7 space-y-6">
      ...
      {/* 🌟 3. Month Switcher Bar */}
      <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-2.5 sm:p-3 flex items-center justify-between shadow-xs">
        <button
          type="button"
          onClick={() => handleMonthChange("prev")}
          disabled={isPending}
          className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-700 hover:bg-slate-100 active:scale-95 transition disabled:opacity-50 cursor-pointer"
        ...
        <div className="flex items-center gap-2 font-bold text-sm sm:text-base text-slate-800">
          ...
          {isPending && (
            <span className="w-3.5 h-3.5 border-2 border-emerald-600 border-t-transparent rounded-full animate-spin ml-1" />
          )}
        </div>
        ...
        <div className="lg:col-span-4 xl:col-span-3 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base sm:text-lg font-bold text-slate-900 flex items-center gap-1.5">
              ...
              <span className="text-xs font-normal text-slate-400">Statistics</span>
            </h3>
          </div>
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/90 shadow-xs space-y-3">
            ...
            <div className="space-y-1 pt-1">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-slate-600">
                  Worst (วันที่ {data.worstDay?.day || "-"})
                </span>
